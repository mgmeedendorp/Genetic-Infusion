...
trait EntityLivingMethodOverrides extends EntityLiving with ISoulEntity {
    ...
    def callMethod[A](name: TypedName[A], superMethod: () => A, args: Any*): A = {
        if(methodLogic != null) {
            methodLogic.callMethod(name, superMethod, args)
        } else if(GIApiInterface.entityMethodRegistry.hasMethodForName(name)) { //methodLogic is null in super constructor call.
            GIApiInterface.entityMethodRegistry.getMethodsForName(name).foreach(method => {
                val returnValue = method.callMethod(this, superMethod, args)
