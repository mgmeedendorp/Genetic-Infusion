...
class TraitRender extends Trait {
    ...
    override def render(entity: IEntitySoulCustom, timeModifier: Float, limbSwing: Float, specialRotation: Float, rotationYawHead: Float, rotationPitch: Float, scale: Float) {
        ...
        if(entity.asInstanceOf[EntityLiving].isChild) {
            ...
            if(model.getCuboidsWithTag(CuboidTypes.Tags.Head).nonEmpty) {
                ...
                for(cuboid <- model.getCuboidsWithTag(CuboidTypes.Tags.Head).get) {
                    cuboid.render()
                }
    ...
    override def renderEquippedItems(entity: IEntitySoulCustom, partialTickTime: Float) {
        ...
        val model: Model = SoulHelper.geneRegistry.getValueFromAllele(entity, Genes.GeneModelAdult)

        GL11.glColor3f(1.0F, 1.0F, 1.0F)
        val itemInHand = living.getHeldItem
        val helmet = living.func_130225_q(3)

        //TODO fix this all in a good way

//        var item: Item = null
//        var f1: Float = 0.0f
//        if (helmet != null) {
//            GL11.glPushMatrix()
//            val head = model.getCuboidsWithTag(CuboidTypes.Tags.Head)
//            head.foreach(head => head.foreach(head => head.applyTranslationAndRotations(0.0625F)))
//            item = helmet.getItem
...
//                }
//                RenderManager.instance.itemRenderer.renderItem(living, helmet, 0)
//            } else if (item == Items.skull) {
//                f1 = 1.0625F
//                GL11.glScalef(f1, -f1, -f1)
//                var gameprofile: GameProfile = null
//                if (helmet.hasTagCompound) {
//                    val nbttagcompound = helmet.getTagCompound
...
//                    }
//                }
//                TileEntitySkullRenderer.field_147536_b.func_152674_a(-0.5F, 0.0F, -0.5F, 1, 180.0F, helmet.getMetadata, gameprofile)
//            }
//            GL11.glPopMatrix()
//        }
...
//                GL11.glScalef(f1, f1, f1)
//            }
//            val rightArms = model.getPartsWithTag(CuboidTypes.Names.Arm, CuboidTypes.Tags.Right)
//
//            if(!living.isDead && rightArms.nonEmpty) {
//                val rightArm = rightArms.get(0)
//                rightArm.postRender(0.0625F)
//                GL11.glTranslatef(-0.0625F, 0.4375F, 0.0625F)
//                val customRenderer = MinecraftForgeClient.getItemRenderer(itemInHand, IItemRenderer.ItemRenderType.EQUIPPED)
//                val is3D = customRenderer != null && customRenderer.shouldUseRenderHelper(IItemRenderer.ItemRenderType.EQUIPPED, itemInHand, IItemRenderer.ItemRendererHelper.BLOCK_3D)
//                if(item.isInstanceOf[ItemBlock] &&
//                    (is3D ||
//                        RenderBlocks.renderItemIn3d(Block.getBlockFromItem(item).getRenderType))) {
//                    f1 = 0.5F
...
//                    i = 0
//                    while(i < itemInHand.getItem.getRenderPasses(itemInHand.getMetadata)) {
//                        val j = itemInHand.getItem.getColorFromItemStack(itemInHand, i)
//                        f5 = (j >> 16 & 255).toFloat / 255.0F
//                        f2 = (j >> 8 & 255).toFloat / 255.0F
//                        val f3 = (j & 255).toFloat / 255.0F
//                        GL11.glColor4f(f5, f2, f3, 1.0F)
//                        RenderManager.instance.itemRenderer.renderItem(living, itemInHand, i)
//                        i += 1
//                    }
//                } else {
//                    i = itemInHand.getItem.getColorFromItemStack(itemInHand, 0)
//                    val f4 = (i >> 16 & 255).toFloat / 255.0F
//                    f5 = (i >> 8 & 255).toFloat / 255.0F
//                    f2 = (i & 255).toFloat / 255.0F
//                    GL11.glColor4f(f4, f5, f2, 1.0F)
//                    RenderManager.instance.itemRenderer.renderItem(living, itemInHand, 0)
//                }
//            }
//            GL11.glPopMatrix()
//        }
