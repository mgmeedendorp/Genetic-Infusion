package seremis.soulcraft.networknew;
...
import seremis.soulcraft.networknew.packet.AbstractPacket;
...
public class PacketPipeline extends MessageToMessageCodec<FMLProxyPacket, AbstractPacket> {
    ...
    public boolean registerPacket(Class<? extends AbstractPacket> clazz) {
        if (this.packets.size() > 256) {
            SoulCraft.logger.error("Soulcraft registered too many packets! 256 is a lot isn't it? packet: " + clazz.getName());
            return false;
        }

        if (this.packets.contains(clazz)) {
            SoulCraft.logger.error("Packet is already registered! packet: " + clazz.getName());
            return false;
        }

        if (this.isPostInitialised) {
            SoulCraft.logger.error("Packet failed to get to the finish line in time! Register packets in init() or preInit()! packet: " + clazz.getName());
            return false;
        }
