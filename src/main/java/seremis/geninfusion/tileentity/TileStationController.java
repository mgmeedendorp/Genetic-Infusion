...
public class TileStationController extends GITileMagnetConsumer implements IInventory, ISidedInventory, IStructureChangeReceiver {
    ...
    public void setTileDataFromClient(int id, byte[] data) {
        ...
        if(id == 1) {
            if(data[0] == 0) {
                if(player != null) {
                    player.openGui(GeneticInfusion.instance, GuiIds.GUI_STATION_SEND_SCREEN_ID, worldObj, xCoord, yCoord, zCoord);
                }
            }
            if(data[0] == 1) {
                if(player != null) {
                    player.openGui(GeneticInfusion.instance, GuiIds.GUI_STATION_TRANSPORTER_SCREEN_ID, worldObj, xCoord, yCoord, zCoord);
                }
            }
        }
        if(id == 2) {
