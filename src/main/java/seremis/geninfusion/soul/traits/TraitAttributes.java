...
public class TraitAttributes extends Trait {
    ...
    public void onInit(IEntitySoulCustom entity) {
        ...
        float maxHealth = ((AlleleFloat) GeneRegistry.getActiveFor(entity, Genes.GENE_MAX_HEALTH)).value;
        entity.getAttributeMap().getAttributeInstance(SharedMonsterAttributes.maxHealth).setBaseValue(maxHealth);

        float attackDamage = ((AlleleFloat) GeneRegistry.getActiveFor(entity, Genes.GENE_ATTACK_DAMAGE)).value;
        entity.getAttributeMap().getAttributeInstance(SharedMonsterAttributes.attackDamage).setBaseValue(attackDamage);

        float movementSpeed = ((AlleleFloat) GeneRegistry.getActiveFor(entity, Genes.GENE_MOVEMENT_SPEED)).value;
        entity.getAttributeMap().getAttributeInstance(SharedMonsterAttributes.movementSpeed).setBaseValue(movementSpeed);

        float knockbackResistance = ((AlleleFloat) GeneRegistry.getActiveFor(entity, Genes.GENE_KNOCKBACK_RESISTANCE)).value;
        entity.getAttributeMap().getAttributeInstance(SharedMonsterAttributes.knockbackResistance).setBaseValue(knockbackResistance);

        float followRange = ((AlleleFloat) GeneRegistry.getActiveFor(entity, Genes.GENE_FOLLOW_RANGE)).value;
    ...
    public void firstTick(IEntitySoulCustom entity) {
        entity.setVariable("creatureAttribute", ((AlleleInteger) GeneRegistry.getActiveFor(entity, Genes.GENE_CREATURE_ATTRIBUTE)).value);
