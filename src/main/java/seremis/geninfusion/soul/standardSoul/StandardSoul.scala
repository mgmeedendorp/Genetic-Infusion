...
abstract class StandardSoul extends IStandardSoul {
    ...
    override def getChromosomeFromGene(entity: EntityLiving, gene: String): IChromosome = {
        ...
        if(gene == Genes.GENE_IS_CREATURE)
            return new Chromosome(new AlleleBoolean(false, entity.isInstanceOf[EntityCreature]))
        ...
        if(gene == Genes.GENE_IS_TAMEABLE)
            return new Chromosome(new AlleleBoolean(true, entity.isInstanceOf[EntityTameable]))
        ...
        if(gene == Genes.GENE_EQUIPMENT_DROP_CHANCES)
            return new Chromosome(new AlleleFloatArray(true, Array(0.085F, 0.085F, 0.085F, 0.085F, 0.085F)))
        ...
        if(gene == Genes.GENE_ITEM_DROPS)
            return new Chromosome(new AlleleInventory(true, Array(null.asInstanceOf[ItemStack])))
        ...
        if(gene == Genes.GENE_RARE_ITEM_DROP_CHANCES)
            return new Chromosome(new AlleleFloatArray(true, Array(0.33F, 0.33F, 0.33F)))
        ...
        if(gene == Genes.GENE_AI_ATTACK_ON_COLLIDE_LONG_MEMORY)
            return new Chromosome(new AlleleBooleanArray(false, Array(false)))
        if(gene == Genes.GENE_AI_ATTACK_ON_COLLIDE_MOVE_SPEED)
            return new Chromosome(new AlleleDoubleArray(false, Array(1.0D)))
        ...
        if(gene == Genes.GENE_AI_AVOID_ENTITY_INDEX)
            return new Chromosome(new AlleleIntArray(false, Array(3)))
        if(gene == Genes.GENE_AI_AVOID_ENTITY_FAR_SPEED)
            return new Chromosome(new AlleleDoubleArray(false, Array(1.0D)))
        if(gene == Genes.GENE_AI_AVOID_ENTITY_NEAR_SPEED)
            return new Chromosome(new AlleleDoubleArray(false, Array(1.2D)))
        if(gene == Genes.GENE_AI_AVOID_ENTITY_RANGE)
            return new Chromosome(new AlleleFloatArray(false, Array(6.0F)))
        if(gene == Genes.GENE_AI_AVOID_ENTITY_TARGET)
            return new Chromosome(new AlleleClassArray(false, Array(classOf[EntityCreeper])))
        ...
        if(gene == Genes.GENE_AI_TARGET_NON_TAMED_INDEX)
            return new Chromosome(new AlleleIntArray(true, Array(1)))
        if(gene == Genes.GENE_AI_TARGET_NON_TAMED_TARGET)
            return new Chromosome(new AlleleClassArray(true, Array(classOf[EntityChicken])))
        if(gene == Genes.GENE_AI_TARGET_NON_TAMED_TARGET_CHANCE)
            return new Chromosome(new AlleleIntArray(true, Array(750)))
        ...
        if(gene == Genes.GENE_AI_TEMPT_ITEM)
            return new Chromosome(new AlleleInventory(true, Array(new ItemStack(Items.wheat))))
        if(gene == Genes.GENE_AI_TEMPT_MOVE_SPEED)
            return new Chromosome(new AlleleDoubleArray(true, Array(1.2D)))
        ...
        if(gene == Genes.GENE_AI_WATCH_CLOSEST_INDEX)
            return new Chromosome(new AlleleIntArray(true, Array(6)))
        ...
        if(gene == Genes.GENE_AI_WATCH_CLOSEST_2_INDEX)
            return new Chromosome(new AlleleIntArray(true, Array(9)))
        if(gene == Genes.GENE_AI_WATCH_CLOSEST_2_CHANCE)
            return new Chromosome(new AlleleFloatArray(true, Array(1.0F)))
        if(gene == Genes.GENE_AI_WATCH_CLOSEST_2_RANGE)
            return new Chromosome(new AlleleFloatArray(true, Array(3.0F)))
