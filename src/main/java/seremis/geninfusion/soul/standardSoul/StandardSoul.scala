...
abstract class StandardSoul extends IStandardSoul {
    ...
    override def getChromosomeFromGene(entity: EntityLiving, gene: String): IChromosome = {
        ...
        if(gene == Genes.GENE_ATTACK_DAMAGE)
            return new Chromosome(new Allele(true, entity.getAttributeMap.getAttributeInstance(SharedMonsterAttributes.attackDamage).getBaseValue))
        if(gene == Genes.GENE_CREATURE_ATTRIBUTE)
            return new Chromosome(new Allele(true, entity.getCreatureAttribute.ordinal()))
        if(gene == Genes.GENE_FOLLOW_RANGE)
            return new Chromosome(new Allele(true, entity.getAttributeMap.getAttributeInstance(SharedMonsterAttributes.followRange).getBaseValue))
        if(gene == Genes.GENE_IMMUNE_TO_FIRE)
            return new Chromosome(new Allele(true, entity.isImmuneToFire))
        if(gene == Genes.GENE_INVULNERABLE)
            return new Chromosome(new Allele(true, entity.isEntityInvulnerable))
        //TODO remove is_creature gene
        if(gene == Genes.GENE_IS_CREATURE)
            return new Chromosome(new Allele(false, entity.isInstanceOf[EntityCreature]))
        if(gene == Genes.GENE_KNOCKBACK_RESISTANCE)
            return new Chromosome(new Allele(true, entity.getAttributeMap.getAttributeInstance(SharedMonsterAttributes.knockbackResistance).getBaseValue))
        if(gene == Genes.GENE_MAX_HEALTH)
            return new Chromosome(new Allele(true, entity.getAttributeMap.getAttributeInstance(SharedMonsterAttributes.maxHealth).getBaseValue))
        if(gene == Genes.GENE_MAX_HURT_RESISTANT_TIME)
            return new Chromosome(new Allele(true, entity.maxHurtResistantTime))
        if(gene == Genes.GENE_MOVEMENT_SPEED)
            return new Chromosome(new Allele(true, entity.getEntityAttribute(SharedMonsterAttributes.movementSpeed).getBaseValue))
        if(gene == Genes.GENE_PORTAL_COOLDOWN)
            return new Chromosome(new Allele(true, entity.getPortalCooldown))
        if(gene == Genes.GENE_TALK_INTERVAL)
            return new Chromosome(new Allele(true, entity.getTalkInterval))
        if(gene == Genes.GENE_TELEPORT_TIME_IN_PORTAL)
            return new Chromosome(new Allele(true, entity.getMaxInPortalTime))
        if(gene == Genes.GENE_VERTICAL_FACE_SPEED)
            return new Chromosome(new Allele(true, entity.getVerticalFaceSpeed))
        //TODO better isTameable check
        if(gene == Genes.GENE_IS_TAMEABLE)
            return new Chromosome(new Allele(true, entity.isInstanceOf[EntityTameable]))
        ...
        if(gene == Genes.GENE_DEATH_SOUND)
            return new Chromosome(new Allele(false, "game.hostile.die"))
        ...
        if(gene == Genes.GENE_EQUIPMENT_DROP_CHANCES)
            return new Chromosome(new Allele(true, Array(0.085F, 0.085F, 0.085F, 0.085F, 0.085F)))
        if(gene == Genes.GENE_EXPERIENCE_VALUE)
            return new Chromosome(new Allele(true, 5))
        if(gene == Genes.GENE_HURT_SOUND)
            return new Chromosome(new Allele(true, "mob.hostile.hurt"))
        if(gene == Genes.GENE_ITEM_DROPS)
            return new Chromosome(new Allele(true, Array(null.asInstanceOf[ItemStack])))
        if(gene == Genes.GENE_LIVING_SOUND)
            return new Chromosome(new Allele(false, null))
        if(gene == Genes.GENE_PICKS_UP_ITEMS)
            return new Chromosome(new Allele(true, false))
        if(gene == Genes.GENE_RARE_ITEM_DROP_CHANCES)
            return new Chromosome(new Allele(true, Array(0.33F, 0.33F, 0.33F)))
        ...
        if(gene == Genes.GENE_SOUND_VOLUME)
            return new Chromosome(new Allele(true, 1F))
        if(gene == Genes.GENE_SPLASH_SOUND)
            return new Chromosome(new Allele(true, "game.neutral.swim.splash"))
        if(gene == Genes.GENE_SWIM_SOUND)
            return new Chromosome(new Allele(true, "game.neutral.swim"))
        if(gene == Genes.GENE_WALK_SOUND)
            return new Chromosome(new Allele(true, "mob.neutral.step"))
        ...
        if(gene == Genes.GENE_AI_ARROW_ATTACK_MAX_RANGED_ATTACK_TIME)
            return new Chromosome(new Allele(false, 0))
        ...
        if(gene == Genes.GENE_AI_ARROW_ATTACK_RANGED_ATTACK_TIME_MODIFIER)
            return new Chromosome(new Allele(false, 1.0F))
        ...
        if(gene == Genes.GENE_AI_ATTACK_ON_COLLIDE_LONG_MEMORY)
            return new Chromosome(new Allele(false, Array(false)))
        if(gene == Genes.GENE_AI_ATTACK_ON_COLLIDE_MOVE_SPEED)
            return new Chromosome(new Allele(false, Array(1.0D)))
        ...
        if(gene == Genes.GENE_AI_AVOID_ENTITY_INDEX)
            return new Chromosome(new Allele(false, Array(3)))
        if(gene == Genes.GENE_AI_AVOID_ENTITY_FAR_SPEED)
            return new Chromosome(new Allele(false, Array(1.0D)))
        if(gene == Genes.GENE_AI_AVOID_ENTITY_NEAR_SPEED)
            return new Chromosome(new Allele(false, Array(1.2D)))
        if(gene == Genes.GENE_AI_AVOID_ENTITY_RANGE)
            return new Chromosome(new Allele(false, Array(6.0F)))
        if(gene == Genes.GENE_AI_AVOID_ENTITY_TARGET)
            return new Chromosome(new Allele(false, Array(classOf[EntityCreeper])))
        ...
        if(gene == Genes.GENE_AI_BEG_RANGE)
            return new Chromosome(new Allele(false, 0.0F))
        ...
        if(gene == Genes.GENE_AI_FOLLOW_OWNER_MIN_DISTANCE)
            return new Chromosome(new Allele(false, 5.0F))
        if(gene == Genes.GENE_AI_FOLLOW_OWNER_MAX_DISTANCE)
            return new Chromosome(new Allele(true, 10.0F))
        ...
        if(gene == Genes.GENE_AI_LEAP_AT_TARGET_MOTION_Y)
            return new Chromosome(new Allele(false, 0.4F))
        ...
        if(gene == Genes.GENE_AI_MOVE_TOWARDS_TARGET_MAX_DISTANCE)
            return new Chromosome(new Allele(true, 32.0F))
        if(gene == Genes.GENE_AI_MOVE_TOWARDS_TARGET_MOVE_SPEED)
            return new Chromosome(new Allele(true, 0.9D))
        ...
        if(gene == Genes.GENE_AI_PANIC_MOVE_SPEED)
            return new Chromosome(new Allele(true, 1.25D))
        ...
        if(gene == Genes.GENE_AI_TARGET_NON_TAMED_INDEX)
            return new Chromosome(new Allele(true, Array(1)))
        if(gene == Genes.GENE_AI_TARGET_NON_TAMED_TARGET)
            return new Chromosome(new Allele(true, Array(classOf[EntityChicken])))
        if(gene == Genes.GENE_AI_TARGET_NON_TAMED_TARGET_CHANCE)
            return new Chromosome(new Allele(true, Array(750)))
        ...
        if(gene == Genes.GENE_AI_TEMPT_ITEM)
            return new Chromosome(new Allele(true, Array(new ItemStack(Items.wheat))))
        if(gene == Genes.GENE_AI_TEMPT_MOVE_SPEED)
            return new Chromosome(new Allele(true, Array(1.2D)))
        ...
        if(gene == Genes.GENE_AI_WATCH_CLOSEST_INDEX)
            return new Chromosome(new Allele(true, Array(6)))
        ...
        if(gene == Genes.GENE_AI_WATCH_CLOSEST_2_INDEX)
            return new Chromosome(new Allele(true, Array(9)))
        if(gene == Genes.GENE_AI_WATCH_CLOSEST_2_CHANCE)
            return new Chromosome(new Allele(true, Array(1.0F)))
        if(gene == Genes.GENE_AI_WATCH_CLOSEST_2_RANGE)
            return new Chromosome(new Allele(true, Array(3.0F)))
