...
class StandardSoulZombie extends StandardSoul {
    ...
    override def getChromosomeFromGene(entity: EntityLiving, gene: String): IChromosome = {
        if(gene.equals(Genes.GENE_BURNS_IN_DAYLIGHT))
            return new Chromosome(new Allele[Boolean](false, true))
        if(gene.equals(Genes.GENE_DEATH_SOUND))
            return new Chromosome(new Allele[String](true, "mob.zombie.death"))
        if(gene.equals(Genes.GENE_HURT_SOUND))
            return new Chromosome(new Allele[String](false, "mob.zombie.hurt"))
        if(gene.equals(Genes.GENE_ITEM_DROPS))
            return new Chromosome(new Allele[Array[ItemStack]](false, Array(new ItemStack(Items.rotten_flesh))))
        if(gene.equals(Genes.GENE_LIVING_SOUND))
            return new Chromosome(new Allele[String](false, "mob.zombie.say"))
        if(gene.equals(Genes.GENE_PICKS_UP_ITEMS))
            return new Chromosome(new Allele[Boolean](false, true))
        if(gene.equals(Genes.GENE_RARE_ITEM_DROPS))
            return new Chromosome(new Allele[Array[ItemStack]](false, Array(new ItemStack(Items.iron_ingot), new ItemStack(Items.carrot), new ItemStack(Items.potato))))
        if(gene.equals(Genes.GENE_SET_ON_FIRE_FROM_ATTACK))
            return new Chromosome(new Allele[Boolean](false, true))
        if(gene.equals(Genes.GENE_SPLASH_SOUND))
            return new Chromosome(new Allele[String](false, "game.hostile.swim.splash"))
        if(gene.equals(Genes.GENE_SWIM_SOUND))
            return new Chromosome(new Allele[String](false, "game.hostile.swim"))
        if(gene.equals(Genes.GENE_WALK_SOUND))
            return new Chromosome(new Allele[String](true, "mob.zombie.step"))
        if(gene.equals(Genes.GENE_USE_NEW_AI))
            return new Chromosome(new Allele[Boolean](true, true))
        if(gene.equals(Genes.GENE_USE_OLD_AI))
            return new Chromosome(new Allele[Boolean](true, false))

        if(gene.equals(Genes.GENE_AI_SWIMMING))
            return new Chromosome(new Allele[Boolean](true, true))
        if(gene.equals(Genes.GENE_AI_SWIMMING_INDEX))
            return new Chromosome(new Allele[Int](true, 0))

        if(gene.equals(Genes.GENE_AI_ATTACK_ON_COLLIDE))
            return new Chromosome(new Allele[Boolean](true, true))
        if(gene.equals(Genes.GENE_AI_ATTACK_ON_COLLIDE_INDEX))
            return new Chromosome(new Allele[Array[Int]](true, Array(2, 4)))
        if(gene.equals(Genes.GENE_AI_ATTACK_ON_COLLIDE_LONG_MEMORY))
            return new Chromosome(new Allele[Array[Boolean]](true, Array(false, true)))
        if(gene.equals(Genes.GENE_AI_ATTACK_ON_COLLIDE_MOVE_SPEED))
            return new Chromosome(new Allele[Array[Double]](true, Array(1.0D, 1.0D)))
        if(gene.equals(Genes.GENE_AI_ATTACK_ON_COLLIDE_TARGET))
            return new Chromosome(new Allele[Array[Class[_]]](true, Array(classOf[EntityPlayer], classOf[EntityVillager])))

        if(gene.equals(Genes.GENE_AI_MOVE_TOWARDS_RESTRICTION))
            return new Chromosome(new Allele[Boolean](true, true))
        if(gene.equals(Genes.GENE_AI_MOVE_TOWARDS_RESTRICTION_INDEX))
            return new Chromosome(new Allele[Int](true, 5))
        if(gene.equals(Genes.GENE_AI_MOVE_TOWARDS_RESTRICTION_MOVE_SPEED))
            return new Chromosome(new Allele[Double](false, 1.0D))

        if(gene.equals(Genes.GENE_AI_MOVE_THROUGH_VILLAGE))
            return new Chromosome(new Allele[Boolean](true, true))
        if(gene.equals(Genes.GENE_AI_MOVE_THROUGH_VILLAGE_INDEX))
            return new Chromosome(new Allele[Int](true, 6))
        ...
        if(gene.equals(Genes.GENE_AI_MOVE_THROUGH_VILLAGE_MOVE_SPEED))
            return new Chromosome(new Allele[Double](true, 1.0D))

        if(gene.equals(Genes.GENE_AI_WANDER))
            return new Chromosome(new Allele[Boolean](true, true))
        if(gene.equals(Genes.GENE_AI_WANDER_INDEX))
            return new Chromosome(new Allele[Int](true, 7))
        ...
        if(gene.equals(Genes.GENE_AI_WATCH_CLOSEST_INDEX))
            return new Chromosome(new Allele[Array[Int]](true, Array(8)))
        if(gene.equals(Genes.GENE_AI_WATCH_CLOSEST_CHANCE))
            return new Chromosome(new Allele[Array[Float]](true, Array(0.02F)))
        if(gene.equals(Genes.GENE_AI_WATCH_CLOSEST_RANGE))
            return new Chromosome(new Allele[Array[Float]](true, Array(8.0F)))
        if(gene.equals(Genes.GENE_AI_WATCH_CLOSEST_TARGET))
            return new Chromosome(new Allele[Array[Class[_]]](true, Array(classOf[EntityPlayer])))

        if(gene.equals(Genes.GENE_AI_LOOK_IDLE))
            return new Chromosome(new Allele[Boolean](true, true))
        if(gene.equals(Genes.GENE_AI_LOOK_IDLE_INDEX))
            return new Chromosome(new Allele[Int](true, 8))

        if(gene.equals(Genes.GENE_AI_HURT_BY_TARGET))
            return new Chromosome(new Allele[Boolean](true, true))
        if(gene.equals(Genes.GENE_AI_HURT_BY_TARGET_INDEX))
            return new Chromosome(new Allele[Int](true, 1))
        ...
        if(gene.equals(Genes.GENE_AI_NEAREST_ATTACKABLE_TARGET_ENTITY_SELECTOR))
            return new Chromosome(new Allele[Array[String]](true, Array("", "")))
        if(gene.equals(Genes.GENE_AI_NEAREST_ATTACKABLE_TARGET_NEARBY_ONLY))
            return new Chromosome(new Allele[Array[Boolean]](true, Array(false, false)))
        if(gene.equals(Genes.GENE_AI_NEAREST_ATTACKABLE_TARGET_TARGET))
            return new Chromosome(new Allele[Array[Class[_]]](false, Array(classOf[EntityPlayer], classOf[EntityVillager])))
        if(gene.equals(Genes.GENE_AI_NEAREST_ATTACKABLE_TARGET_TARGET_CHANCE))
            return new Chromosome(new Allele[Array[Int]](true, Array(0, 0)))
        if(gene.equals(Genes.GENE_AI_NEAREST_ATTACKABLE_TARGET_VISIBLE))
            return new Chromosome(new Allele[Array[Boolean]](true, Array(true, false)))

        if(gene.equals(Genes.GENE_MODEL))
            return new Chromosome(new Allele[Array[ModelPart]](true, ModelPart.getModelPartsFromModel(new ModelZombie(), entity)))
        if(gene.equals(Genes.GENE_TEXTURE))
            return new Chromosome(new Allele[String](true, "textures/entity/zombie/zombie.png"), new Allele[String](false, "textures/entity/zombie/zombie.png"))
